...
const getUnverifiedInstitutes = async (hq_type) => {
    ...
    if (SERVICE_ENUM.INSTITUTE_TYPES.Police_HQ === hq_type){
        queryString = `SELECT p.institute_id, p.addr_line_1, p.addr_line_2, p.city, p.district, p.province, 
                        pc.station_category,p.motor_vehicles,p.motor_biycles,p.officers,p.weapons, p.cells, ST_AsGeoJSON(p.location) from police p, 
        ...
        queryString = `SELECT h.institute_id, h.addr_line_1, h.addr_line_2, h.city, h.district, h.province, hc.hospital_category, h.icu_beds, h.doctors,
                        h.ambulances, h.capacity,ST_AsGeoJSON(h.location) from hospital h, government_institute g, hospital_category hc where g.institute_status = 1 and
        ...
        queryString = `SELECT f.institute_id, f.addr_line_1, f.addr_line_2, f.city, f.district, f.province, f.fire_trucks, f.fire_fighters,ST_AsGeoJSON(f.location) from firestation f,
...
const analyzeDate = async (accountType, startDate, endDate) => {
    let category;
    if(accountType === SERVICE_ENUM.INSTITUTE_TYPES.Police_HQ){
        category = SERVICE_ENUM.INSTITUTE_TYPES.Police_station;
    }else if(accountType === SERVICE_ENUM.INSTITUTE_TYPES.Hospital_HQ){
...
const analyzeProvince = async (accountType, startDate, endDate) => {
    let category;
    if(accountType === SERVICE_ENUM.INSTITUTE_TYPES.Police_HQ){
        category = SERVICE_ENUM.INSTITUTE_TYPES.Police_station;
    }else if(accountType === SERVICE_ENUM.INSTITUTE_TYPES.Hospital_HQ){
